...
function toOutputFilePathInHtml({ filename, type, hostId, hostType, config, toRelative }) {
	...
	else return joinUrlSegments(config["decodedBase"] ?? config.base, filename);
}
...
function joinUrlSegments(a, b) {
	if (!a || !b) return a || b || "";
	if (a.endsWith("/")) a = a.slice(0, Math.max(0, a.length - 1));
	if (b[0] !== "/") b = "/" + b;
	return a + b;
}
function toAssetPathFromHtml(filename, htmlPath, config) {
	const relativeUrlPath = normalizePath(path.relative(config.root, htmlPath));
	const toRelative = (filePath, hostId) => getBaseInHTML(relativeUrlPath, config) + filePath;
...
const modernTargetsSwc = "edge>=79, firefox>=67, chrome>=64, safari>=12, chromeAndroid>=64, iOS>=12";
function viteLegacyPlugin(options = {}) {
	let resolvedConfig;
	let targets;
	const modernTargets = options.modernTargets || modernTargetsSwc;
	...
	const legacyConfigPlugin = {
		...
		async config(config, env$1) {
			if (env$1.command === "build" && !config.build?.ssr) {
			...
			return { define: { "import.meta.env.LEGACY": env$1.command === "serve" || config.build?.ssr ? false : legacyEnvVarMarker } };
		},
		configResolved(config) {
			if (overriddenBuildTarget) config.logger.warn(import_picocolors.default.yellow(`vite-plugin-legacy-swc overrode 'build.target'. You should pass 'targets' as an option to this plugin with the list of legacy browsers to support instead.`));
			if (overriddenBuildTargetOnlyModern) config.logger.warn(import_picocolors.default.yellow(`vite-plugin-legacy-swc overrode 'build.target'. You should pass 'modernTargets' as an option to this plugin with the list of browsers to support instead.`));
			if (overriddenDefaultModernTargets) config.logger.warn(import_picocolors.default.yellow(`vite-plugin-legacy-swc 'modernTargets' option overrode the builtin targets of modern chunks. Some versions of browsers between legacy and modern may not be supported.`));
			if (config.isWorker) config.logger.warn(import_picocolors.default.yellow(`vite-plugin-legacy-swc should not be passed to 'worker.plugins'. Pass to 'plugins' instead. Note that generating legacy chunks for workers are not supported by vite-plugin-legacy-swc.`));
		}
	};
	const legacyGenerateBundlePlugin = {
		...
		async generateBundle(opts, bundle) {
			...
			if (!chunkFileNameToPolyfills) throw new Error("Internal vite-plugin-legacy-swc error: discovered polyfills should exist");
			if (!isLegacyBundle(bundle, opts)) {
				...
				if (isDebug) console.log(`[vite-plugin-legacy-swc] modern polyfills:`, modernPolyfills);
			...
			if (legacyPolyfills.size || !options.externalSystemJS) {
				if (isDebug) console.log(`[vite-plugin-legacy-swc] legacy polyfills:`, legacyPolyfills);
				await buildPolyfillChunk({
					mode: resolvedConfig.mode,
	...
	const legacyPostPlugin = {
		...
		configResolved(config) {
			if (config.build.lib) throw new Error("vite-plugin-legacy-swc does not support library mode.");
			resolvedConfig = config;
			if (isDebug) console.log(`[vite-plugin-legacy-swc] modernTargets:`, modernTargets);
			if (!genLegacy || resolvedConfig.build.ssr) return;
			targets = options.targets || browserslistLoadConfig({ path: resolvedConfig.root }) || "last 2 versions and not dead, > 0.3%, Firefox ESR";
			if (isDebug) console.log(`[vite-plugin-legacy-swc] targets:`, targets);
			const getLegacyOutputFileName = (fileNames, defaultFileName = "[name]-legacy-[hash].js") => {
				if (!fileNames) return path.posix.join(resolvedConfig.build.assetsDir, defaultFileName);
				return (chunkInfo) => {
			...
			const createLegacyOutput = (outputOptions = {}) => {
				return {
					...outputOptions,
					format: "system",
		...
		async renderChunk(raw, chunk, opts, { chunks }) {
			if (resolvedConfig.build.ssr) return null;
			...
			if (!chunkFileNameToPolyfills) {
				...
				for (const fileName of Object.keys(chunks)) chunkFileNameToPolyfills.set(fileName, {
			...
			if (!polyfillsDiscovered) throw new Error(`Internal vite-plugin-legacy-swc error: discovered polyfills for ${chunk.fileName} should exist`);
			if (!isLegacyChunk(chunk, opts)) {
				...
				if (resolvedConfig.build.sourcemap) return {
					code: ms.toString(),
			...
			const minifyOptions = {
				...
				mangle: true,
				safari10: true,
				...resolvedConfig.build.terserOptions,
			...
			const transformResult = await swc.transform(raw, {
				...
				jsc: {
					...
					transform: { optimizer: { globals: { vars: { [legacyEnvVarMarker]: "true" } } } }
				}
			});
			const plugin = swc.plugins([recordAndRemovePolyfillSwcPlugin(polyfillsDiscovered.legacy), wrapIIFESwcPlugin()]);
		...
		transformIndexHtml(html, { chunk }) {
			...
			if (genModern) {
				...
				if (modernPolyfillFilename) tags.push({
					...
					attrs: {
						type: "module",
						crossorigin: true,
						src: toAssetPathFromHtml(modernPolyfillFilename, chunk.facadeModuleId, resolvedConfig)
					}
				});
			...
			if (legacyPolyfillFilename) tags.push({
				...
				attrs: {
					...
					crossorigin: true,
					id: legacyPolyfillId,
					src: toAssetPathFromHtml(legacyPolyfillFilename, chunk.facadeModuleId, resolvedConfig)
				},
				injectTo: "body"
			});
			...
			if (legacyEntryFilename) tags.push({
				...
				attrs: {
					...
					crossorigin: true,
					id: legacyEntryId,
					"data-src": toAssetPathFromHtml(legacyEntryFilename, chunk.facadeModuleId, resolvedConfig)
				},
		...
		generateBundle(opts, bundle) {
			...
			if (isLegacyBundle(bundle, opts) && genModern) {
				for (const name of Object.keys(bundle)) if (bundle[name].type === "asset" && !name.endsWith(".map")) delete bundle[name];
			}
		}
	};
	return [
...
function createSwcEnvOptions(targets, { needPolyfills = true }) {
	return {
		targets,
		bugfixes: true,
		loose: false,
		mode: needPolyfills ? "usage" : void 0,
		coreJs: needPolyfills ? $require("core-js/package.json").version : void 0,
		shippedProposals: true
	};
}
const polyfillId = "\0vite/legacy-polyfills";
async function buildPolyfillChunk({ mode, imports, bundle, facadeToChunkMap, buildOptions, format, rollupOutputOptions, excludeSystemJS, prependModernChunkLegacyGuard }) {
	...
	const res = await build({
		...
		configFile: false,
		logLevel: "error",
		plugins: [polyfillsPlugin(imports, excludeSystemJS), prependModernChunkLegacyGuard && prependModernChunkLegacyGuardPlugin()],
		build: {
			write: false,
			minify: false,
			assetsDir,
	...
	const polyfillChunk = rollupOutput.output.find((chunk) => chunk.type === "chunk" && chunk.isEntry);
	if (minify) {
		const swc = await loadSwc();
		...
		const minifyResult = await swc.transform(polyfillChunk.code, {
			...
			configFile: false,
			sourceMaps,
			inputSourceMap: polyfillChunk.map ? polyfillChunk.map.toString() : void 0,
			minify: true,
			jsc: { minify: minifyOptions }
		});
	...
	if (polyfillChunk.sourcemapFileName) {
		const polyfillChunkMapAsset = rollupOutput.output.find((chunk) => chunk.type === "asset" && chunk.fileName === polyfillChunk.sourcemapFileName);
...
function prependModernChunkLegacyGuardPlugin() {
...
function recordAndRemovePolyfillSwcPlugin(polyfills) {
	return (program) => {
		program.body = program.body.filter((node) => {
			...
			if (node.type === "VariableDeclaration") {
				node.declarations = node.declarations.filter((declaration) => {
					if (declaration.init && declaration.init.type === "CallExpression" && declaration.init.callee.type === "Identifier" && declaration.init.callee.value === "require" && declaration.init.arguments[0].expression.type === "StringLiteral") {

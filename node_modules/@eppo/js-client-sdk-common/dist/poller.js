...
function initPoller(intervalMs, 
...
callback, options) {
    ...
    const start = async () => {
        ...
        let startAttemptsRemaining = options?.skipInitialPoll
            ? 0
            : 1 + (options?.maxStartRetries ?? constants_1.DEFAULT_INITIAL_CONFIG_REQUEST_RETRIES);
        ...
        const startRegularPolling = !stopped &&
    ...
    async function poll() {
        ...
        try {
            ...
            const maxTries = 1 + (options?.maxPollRetries ?? constants_1.DEFAULT_POLL_CONFIG_REQUEST_RETRIES);

 ...
 */
function createLLMSimulatedUser({ system, model, client, fixedResponses, }) {
    if (!client && !model) {
    ...
    return async function _simulator(params) {
        if (model) {
            ...
            client = await (0, universal_1.initChatModel)(model);
        }
        ...
        for (const msg of params.trajectory) {
            const convertedMessage = (0, utils_js_1._convertToOpenAIMessage)(msg);
...
exports.createLLMSimulatedUser = createLLMSimulatedUser;

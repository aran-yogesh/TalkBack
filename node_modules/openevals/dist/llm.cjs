...
exports.createLLMAsJudge = exports._createLLMAsJudgeScorer = exports.isZodSchema = void 0;
...
function _stringifyPromptParam(param) {
    ...
    else if ((0, messages_1.isBaseMessage)(param)) {
        return JSON.stringify((0, utils_js_1._convertToOpenAIMessage)(param));
    }
    else if (typeof param === "object" && param !== null) {
        if (Array.isArray(param)) {
            return JSON.stringify(param.map((message) => (0, messages_1.isBaseMessage)(message) ? (0, utils_js_1._convertToOpenAIMessage)(message) : message));
        }
        const objParam = param;
        if ("messages" in objParam && Array.isArray(objParam.messages)) {
            objParam.messages = objParam.messages.map((message) => (0, messages_1.isBaseMessage)(message) ? (0, utils_js_1._convertToOpenAIMessage)(message) : message);
...
const _createLLMAsJudgeScorer = (params) => {
    ...
    if (isZodSchema(params.schema)) {
        schema = (0, json_schema_1.toJsonSchema)(params.schema);
    }
    else {
...
function createLLMAsJudge({ prompt, feedbackKey = "score", model, system, judge, continuous = false, choices, useReasoning = true, fewShotExamples, outputSchema, }) {
    ...
    const _wrappedEvaluator = async (inputs) => {
        ...
        return (0, utils_js_1._runEvaluatorUntyped)(runName, scorer, feedbackKey, inputs, undefined, outputSchema !== undefined || _isStructuredPrompt(prompt));
    };

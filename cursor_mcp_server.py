...
execution_results = {"last_run_time": None, "last_output": "", "error_count": 0, "linter_errors": [], "success": False}
...
async def handle_read_resource(uri: str) -> str:
    ...
    if uri == "talkback://execution-results":
        ...
        return json.dumps(
            {"linter_errors": execution_results["linter_errors"], "error_count": execution_results["error_count"]},
            indent=2,
        )
    else:
    ...
    return [
        types.Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "output": {"type": "string", "description": "Terminal output from code execution"},
                    "error_count": {"type": "integer", "description": "Number of errors detected"},
                    "linter_errors": {
                        "type": "array",
        ...
        types.Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {"force": {"type": "boolean", "description": "Force roast even if no recent execution"}},
            },
        ),
    ]
...
async def handle_call_tool(
    ...
    if name == "report_code_execution":
        ...
        if error_count >= 2:
            # ROAST MODE 🔥
            output_snippet = execution_results["last_output"][:500]
            roast_prompt = (
                f"ROAST ME HARD! My code just failed with {error_count} errors. Here's the output: {output_snippet}"
            )
            response_type = "roast"
        elif error_count == 1:
            # Minor sass
            output_snippet = execution_results["last_output"][:300]
            roast_prompt = f"I got 1 error. Give me a little attitude about it: {output_snippet}"
        ...
        if not execution_results["last_run_time"] and not force:
            return [types.TextContent(type="text", text="No recent code execution to roast about!")]
...
async def trigger_talkback_speech(prompt: str, response_type: str):
    ...
    talkback_message = {"prompt": prompt, "type": response_type, "timestamp": time.time()}

...
async def trigger_talkback_speech(prompt: str, response_type: str):
    """Send prompt to TalkBack via the reliable IPC helper."""
    ok = _send_talkback(prompt, response_type)
    if ok:
        print(f"🎤 TalkBack message sent: {response_type}", file=sys.stderr)
    else:
        print(f"❌ TalkBack message FAILED: {response_type}", file=sys.stderr)

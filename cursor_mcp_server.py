...
async def trigger_talkback_speech(prompt: str, response_type: str):
    """Send prompt to TalkBack via file-based IPC (atomic write)."""
    talkback_message = {
        "prompt": prompt,
        "type": response_type,
        "timestamp": time.time()
    ...
    try:
        fd, tmp_path = tempfile.mkstemp(dir="/tmp", suffix=".tmp")
        try:
        ...
        print(f"❌ Failed to send TalkBack message: {e}", file=sys.stderr)

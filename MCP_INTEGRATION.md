...
### Key Files:
- **`cursor_code_monitor.py`**: Code monitor — wraps commands, counts errors, writes roast triggers (requires `watchdog`)
- **`cursor_mcp_server.py`**: Full MCP server for Cursor IDE integration (requires `mcp`)
- **`test_mcp_connection.py`**: Quick test to verify IPC works
- **`broken_code.py`**: Intentionally broken script for testing roast triggers
- **`ConversationalTalkBack.swift`**: Main app with MCP polling built in
...
### Method 2: Cursor IDE Terminal Integration

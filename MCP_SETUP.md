...
Copy `config.swift.template` to `config.swift` and replace the placeholder values with your actual keys:

```bash
cp config.swift.template config.swift
```

Then edit `config.swift` and fill in:
- `YOUR_OPENAI_API_KEY_HERE` with your OpenAI key
- `YOUR_ELEVENLABS_API_KEY_HERE` with your ElevenLabs key

...
- **Continuous listening** — just speak naturally, no button press needed
- Built-in voice activity detection (VAD) with automatic silence handling
...
- Smart noise filtering to ignore background sounds
...
- Hover over any text on screen to preview it in a floating overlay
- **Summarize** or **Make Concise** actions powered by OpenAI
- Activate from the menu bar viewfinder icon or hold ⌥ Option for temporary lens
- Result caching to avoid redundant API calls
- Requires macOS Accessibility permission

### 🎓 **Coding Teacher Mode**
- Enabled by default — toggle from the Lens menu bar item
- Automatically provides teaching feedback after each command execution
- On success: celebrates briefly, explains the result, suggests a next step
- On failure: diagnoses likely causes, teaches what went wrong, gives actionable fixes
- Witty but supportive tone with light sass

### 📬 **Assignment Monitoring**
- Opt-in feature toggled from the menu bar
- Scans macOS Mail.app inbox every 3 minutes for assignment-related emails
- Detects keywords like `assignment`, `homework`, `due`, `quiz`, `exam`
- Matches educational domains (`edu`, `canvas`, `blackboard`)
- Summarizes matched emails with course, due dates, and suggested next actions

### 😴 **Idle Nudges**
- Detects when you've been quiet for 60+ seconds
- Sends playful nudge messages to keep you engaged
- Responses generated by OpenAI for variety
...
3. **Talk to TalkBack**: Just speak naturally — continuous listening is always on. TalkBack detects your voice automatically and stops recording after 2 seconds of silence.
...
1. Click the **viewfinder icon** in the menu bar and enable **Lens Mode**, or hold ⌥ **Option** for a temporary lens
2. Hover over any text element on screen — a floating overlay shows the text
3. Click **Summarize** or **Make Concise** to process the text with OpenAI

### 🎓 Using Teacher Mode

Teacher Mode is on by default. When you run commands through the MCP monitor, TalkBack provides educational feedback instead of (or in addition to) roasts. Toggle it from the **Lens menu bar item → Coding Teacher Mode**.
...
| `cline_mcp_settings.json` | Cline MCP settings (alternative MCP config with venv Python path) |
| `AGENTS.md` | AI agent coding guide and project conventions |

## 📚 Additional Docs

| Document | Description |
|---|---|
| [QUICK_START.md](QUICK_START.md) | 3-step quick start for MCP roasting |
| [MCP_SETUP.md](MCP_SETUP.md) | MCP + Cursor IDE integration overview |
| [MCP_SETUP_COMPLETE.md](MCP_SETUP_COMPLETE.md) | Detailed MCP setup walkthrough with Cursor IDE configuration |
| [MCP_INTEGRATION.md](MCP_INTEGRATION.md) | Architecture deep-dive with pipeline diagrams |
...
- Speak clearly — the VAD threshold filters out quiet sounds
...
- Ensure the app (or Terminal) is listed and enabled in the Accessibility list
...
- Always-on `AVAudioEngine` continuous listening with voice activity detection
- macOS Accessibility API (`AXUIElement`) for Lens Mode text reading
- Rate limiting (22s OpenAI cooldown) to stay within API quotas

...
For API key setup details, see [Installation](#installation) and [API_KEY_SETUP.md](./API_KEY_SETUP.md).

### Additional Setup Docs

- [API Key Setup Guide](./API_KEY_SETUP.md)
- [MCP Setup Guide](./MCP_SETUP.md)
- [MCP Integration Details](./MCP_INTEGRATION.md)
- [Quick Start Notes](./QUICK_START.md)
- [MCP Setup Completion Notes](./MCP_SETUP_COMPLETE.md)
...
These commands verify MCP integration behavior after setup.

### Security Note

Do not commit real API keys to source control. Use `config.swift.template` as a safe reference for local configuration.

### Platform Note

This project is intended for macOS and cannot be built or run natively on Linux.

### Repository Structure

- `ConversationalTalkBack.swift` — main app runtime
- `cursor_code_monitor.py` — terminal monitoring helper for MCP roast flow
- `test_mcp_connection.py` — MCP connectivity check
- `test_mcp_roast.py` — roast behavior test helper
- `start_talkback_mcp.sh` / `start_integration.sh` — startup helpers
- `config.swift.template` — configuration template without secrets
- `mcp_config.json` / `cline_mcp_settings.json` — MCP-related config files

### Notes

- The app requires camera and microphone permissions for full functionality.
- If commands or setup behavior change, keep this README and linked setup docs in sync.
- Commands in this README are designed to run from the repository root.

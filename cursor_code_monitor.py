...
DEPRECATED: This module is superseded by cursor_mcp_server.py which provides
a proper MCP server integration instead of file-based IPC. The built-in Swift
MCP monitoring in ConversationalTalkBack.swift is the recommended approach.
...
warnings.warn(
    ...
    "Use cursor_mcp_server.py or the built-in Swift MCP monitoring instead.",
    DeprecationWarning,
    stacklevel=2,
)

from watchdog.events import FileSystemEventHandler  # noqa: E402
from watchdog.observers import Observer  # noqa: E402, F401
...
class CodeExecutionMonitor(FileSystemEventHandler):
    ...
    def send_to_talkback(self, output: str, error_count: int, success: bool):
        """Send code execution results to TalkBack via file-based IPC (deprecated)."""
        warnings.warn(
            "File-based IPC via /tmp/talkback_message.json is deprecated. "
            "Use the MCP server protocol in cursor_mcp_server.py instead.",
            DeprecationWarning,
            stacklevel=2,
        )
